...
export function TraderPivotTable({
  data,
  rowLabel = "Entidad",
  unit = { short: "mt", per: "mt" },
}: {
  data: TraderMonthlyBreakdown;
  rowLabel?: string;
  ...
  const toggleExpand = (entidad: string) => {
    setExpanded(prev => {
      const next = new Set(prev);
      if (next.has(entidad)) next.delete(entidad);
      else next.add(entidad);
      return next;
    });
  };
    ...
    <div className="space-y-3">
      ...
      >
        <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "max-content", minWidth: "100%" }}>
          <thead>
            <tr>
              <th rowSpan={3} style={{ ...yearHeaderStyle(T), position: "sticky", left: 0, top: 0, zIndex: 4, minWidth: 240, textAlign: "left", backgroundColor: dark ? "#0a2748" : "#06254B" }}>
                {rowLabel} · Producto
              </th>
              {yearGroups.map((yg, i) => (
                <th
