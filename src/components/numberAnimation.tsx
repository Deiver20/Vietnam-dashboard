      ...
      ([entry]) => {
        ...
        if (entry.isIntersecting) {
          tween = gsap.to(fromVal, {
            ...
            ease: "power1.out",
            onUpdate() {
              // The element is gone once React unmounts this component; a
              // tween created by an observer callback dispatched before the
              // effect cleanup would otherwise keep firing against null.
              if (!elRef.current) {
                tween?.kill();
                return;
              }
